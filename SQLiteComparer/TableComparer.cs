...
namespace SQLiteComparer
{
    ...
    /// This comparer will create a TableDifferences class that will be used to access the 
    ...
    public class TableComparer
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TableComparer"/> class.
        /// </summary>
        ...
        /// <param name="compareBlobs">TRUE will cause the comparer to compare BLOB fields, FALSE
        ...
        public TableComparer(SQLiteCreateTableStatement leftTable,
            ...
            using (SQLiteConnection right = new SQLiteConnection(rightstr))
            {
                ...
                using (SQLiteConnection left = new SQLiteConnection(leftstr))
                {
                    ...
                    try
                    {
                        ...
                        SQLiteCommand select = BuildSelectCommand(tableName.ToString(), common, right, rightTx, _rightTable);
            ...
            using (SQLiteConnection left = new SQLiteConnection(leftstr))
            {
                ...
                using (SQLiteConnection right = new SQLiteConnection(rightstr))
                {
                    ...
                    try
                    {
                        ...
                        SQLiteCommand select = BuildSelectCommand(tname, leftCommon, left, leftTx, _leftTable);
        ...
        {
            var id = table.WithoutRowid ? "" : "RowID";

            string clist = Utils.BuildColumnsString(common, true);
            SQLiteCommand select = new SQLiteCommand($"SELECT {id}, " + clist + " FROM " +
                tableName, conn, tx);
            return select;
        }
                ...
                while (rightReader.Read())
                {
                    ...
                    if (haspkey)
                    {
                        ...
                        foreach (SQLiteColumnStatement col in common)
                        {
                            ...
                            {
                                var ltype = Utils.GetDbType(leftCol.ColumnType);
                                var rtype = Utils.GetDbType(rightCol.ColumnType);
            ...
            bool castBlobs)
        {
            ...
            for (int i = 0; i < pkeys.Count; i++)
            {
                ...
                else
                {
                    ...
                    var dbtype = Utils.GetDbTypeFromClrType(value);
