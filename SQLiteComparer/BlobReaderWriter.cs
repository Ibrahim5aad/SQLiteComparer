...
namespace SQLiteComparer
{
    ...
    public class BlobReaderWriter : IDisposable
    {
        ...
        public BlobReaderWriter(string dbpath, bool readOnly)
        {
            ...
            if (_errorCode > 0)
                throw new SQLiteException((SQLiteErrorCode)_errorCode, "can't open file: " + dbpath);
        }
        ...
        public BlobReaderWriter(string dbpath1, string dbpath2)
        {
            ...
            if (_errorCode > 0)
                throw new SQLiteException((SQLiteErrorCode)_errorCode, "can't open file: " + dbpath1);
            _errorCode = sqlite3_open_v2(dbpath2, ref _conn2, SQLITE_OPEN_READONLY, null);
            if (_errorCode > 0)
                throw new SQLiteException((SQLiteErrorCode)_errorCode, "can't open file: " + dbpath2);
        }
        ...
        public void WriteBlob(string tableName, string columnName, long rowId, int blobLength, BlobWriter writer)
        {
            ...
            if (needsZeroing)
            {
                ...
                if (_blob1 != IntPtr.Zero)
                {
                    ...
                    _blob1 = IntPtr.Zero;
                    if (_errorCode > 0)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "can't close BLOB handle");
                }
                ...
                if (_errorCode > 0)
                    throw new SQLiteException((SQLiteErrorCode)_errorCode, "sqlite3_prepare_v2 failed");
                ...
                try
                {
                    ...
                    if (_errorCode > 0)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to bind zero-blob");

                    // Bind the RowID parameter
                    _errorCode = sqlite3_bind_int64(stmt, 2, rowId);
                    if (_errorCode > 0)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to bind rowid");

                    // Execute the prepared statement
                    _errorCode = sqlite3_step(stmt);
                    if (_errorCode > 0 && _errorCode < 100)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to execute zeroblob command");
                }
                finally
                {
                ...
                _errorCode = sqlite3_blob_open(_conn1, "main", tableName, columnName, rowId, 1, ref _blob1);
                if (_errorCode > 0)
                    throw new SQLiteException((SQLiteErrorCode)_errorCode, "sqlite3_blob_open failed");
            } // if
            ...
            try
            {
                ...
                while (written < len)
                {
                    ...
                    if (_errorCode > 0)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "sqlite3_blob_write failed");
        ...
        public void ReadBlob(string tableName, string columnName, long rowId, BlobReader reader)
        {
            ...
            if (_errorCode > 0)
            {
                ...
                if (_errorCode > 0)
                    throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to prepare SELECT statement");
                ...
                try
                {
                    ...
                    _errorCode = sqlite3_step(stmt);
                    if (_errorCode > 0 && _errorCode < 100)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to execute SELECT command");
            ...
            try
            {
                ...
                while (count > 0)
                {
                    ...
                    if (_errorCode > 0)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "sqlite3_blob_read failed");
        ...
        public void CopyBlob(string targetdb, string tableName, string columnName, long fromRowId, long toRowId, BlobProgressHandler handler)
        {
            ...
            if (_errorCode > 0)
                throw new SQLiteException((SQLiteErrorCode)_errorCode, "can't open file: " + targetdb);
            ...
            try
            {
                ...
                if (_errorCode > 0)
                    throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to open BLOB handle");
                ...
                if (needsResizing)
                {
                    ...
                    _errorCode = sqlite3_blob_open(_conn2, "main", tableName, columnName, toRowId, 1, ref _blob2);
                    if (_errorCode > 0)
                        throw new SQLiteException((SQLiteErrorCode)_errorCode, "failed to re-open target BLOB handle");
                } // if
